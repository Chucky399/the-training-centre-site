...
import json, os, re, shutil, sys, time, html, urllib.request
...
# Manifest of the pages this generator owns, written every run and committed.
# Comparing it against the current feed is how a WITHDRAWN course is spotted:
# a slug we generated last run that the feed no longer carries.
MANIFEST = "courses/_generated.json"
# Old URLs of withdrawn courses -> where to send the visitor. Accumulates, and is
# rendered into functions/_withdrawn.js so the edge can 301 instead of 404.
WITHDRAWN_JSON = "courses/_withdrawn.json"
WITHDRAWN_JS = "functions/_withdrawn.js"
# A feed hiccup must never be able to strip the site. More than this many course
# pages disappearing in one run stops the build for a human to look at.
MAX_WITHDRAWALS_PER_RUN = 2
...
document.getElementById('enquiry-form').addEventListener('submit', function(e) {
  e.preventDefault();
  var form = this;
  var btn = form.querySelector('button[type="submit"]'); var btnText = btn.textContent;
  btn.disabled = true; btn.textContent = 'Sending...';
  fetch('https://lp-form-handler.claireljarrett.workers.dev/submit', { method: 'POST', body: new FormData(form) })
    .then(function(r) {
      return r.json().catch(function() { return {}; }).then(function(d) {
        if (r.ok && d.success !== false) { window.location.href = '/thank-you/'; return; }
        btn.disabled = false; btn.textContent = btnText;
        if (window.turnstile) { try { window.turnstile.reset(); } catch (x) {} }
        alert(d.message || 'Something went wrong. Please call us on 020 3858 9207.');
      });
    })
...
<section id="enquire" class="scroll-mt-28 max-w-6xl mx-auto px-5 py-16 grid md:grid-cols-[0.9fr_1.1fr] gap-10">
  ...
  <form id="enquiry-form" action="https://lp-form-handler.claireljarrett.workers.dev/submit" method="POST" class="bg-white border border-[#323F48]/10 rounded-[8px] p-6 sm:p-8 space-y-4">
    ...
    <div aria-hidden="true" style="position:absolute;left:-10000px;top:auto;width:1px;height:1px;overflow:hidden"><label for="ttc-hp-url">Leave this field empty</label><input type="text" id="ttc-hp-url" name="url" tabindex="-1" autocomplete="off"></div>
    ...
    <div class="cf-turnstile" data-sitekey="0x4AAAAAAEz1sVk1JQjANLXo" data-appearance="interaction-only"></div>
...
def reconcile_withdrawals(templates):
    ...
    The page is deleted so it cannot go on selling a course that is no longer run,
    and its address is recorded so the edge redirects it to the category page
    instead of serving a 404 to anyone holding a link or arriving from search.
    """
    live = {}
    for t in templates:
        code = t.get("Code")
    ...
    if len(gone) > MAX_WITHDRAWALS_PER_RUN:
        sys.exit(f"ABORT: {len(gone)} course pages would be retired in one run "
                 f"({', '.join(sorted(gone))}), above the limit of {MAX_WITHDRAWALS_PER_RUN}. "
                 "Nothing deleted - check the Arlo feed before rerunning.")

    wd_path = os.path.join(BASE, WITHDRAWN_JSON)
    withdrawn = {}
    if os.path.exists(wd_path):
        try:
            withdrawn = json.load(open(wd_path, encoding="utf-8"))
        except (ValueError, OSError):
            withdrawn = {}

    for slug in gone:
        target = previous[slug].get("category") or "/courses/"
        if target == f"/courses/{slug}/":
            target = "/courses/"
        withdrawn[f"/courses/{slug}/"] = target
        d = os.path.join(BASE, "courses", slug)
        if os.path.isdir(d):
            shutil.rmtree(d)
            print(f"  retired /courses/{slug}/ -> {target}")

    # A course that comes back must stop redirecting.
    for path in list(withdrawn):
        if path.strip("/").split("/")[-1] in live:
            del withdrawn[path]
    ...
    with open(wd_path, "w", encoding="utf-8", newline="\n") as f:
        json.dump(withdrawn, f, indent=2, sort_keys=True)
        f.write("\n")
    with open(os.path.join(BASE, WITHDRAWN_JS), "w", encoding="utf-8", newline="\n") as f:
        f.write("/* GENERATED by _build_pages.py - do not edit by hand.\n"
        ...
        f.write("export const WITHDRAWN = " + json.dumps(withdrawn, indent=2, sort_keys=True) + ";\n")
    print(f"withdrawals: {len(gone)} retired this run, {len(withdrawn)} redirect(s) live")
...
def main():
    ...
    reconcile_withdrawals(templates)
